...
export default class LedgerAPI {
  ...
  createTransport() {
    ...
    return LedgerAPITransport.create().then((t) => {
      this.transport = t
  ...
  verifyConnect() {
    return this.str.getAppConfiguration().then(() => {
      return null
    }).catch(() => {
  ...
  connect() {
    return this.createTransport().then(() => {
      return this.verifyConnect().catch((result) => {
        console.log('connect failed: ', result)
